...
class VoltageAC_3ph(Element):
    """
    Classe responavel pela adicao de Fontes de Tensao AC Trifasicas.
    """
    def __init__(self, bus_pos, amp, freq=60, tstart=-1, tstop=100, bus_neg=None, hide_c=False):
        ...
        if not self.hide_c:
            if self.bus_neg is None:
                self.source = "C FONTE TENSAO AC 3PH - POS:" + self.bus_pos.name + "\n"
            else:
                self.source = "C FONTE TENSAO AC 3PH - POS:" + self.bus_pos.name + " - NEG: " + self.bus_neg.name + "\n"

        if self.bus_neg is None:
            self.va = VoltageAC(bus_pos=self.bus_pos, amp=self.amp, freq=self.freq, phase=0, tstart=self.tstart,
                                tstop=self.tstop, phase_pos="A", hide_c=True)
            self.vb = VoltageAC(bus_pos=self.bus_pos, amp=self.amp, freq=self.freq, phase=-120, tstart=self.tstart,
                                tstop=self.tstop, phase_pos="B", hide_c=True)
            self.vc = VoltageAC(bus_pos=self.bus_pos, amp=self.amp, freq=self.freq, phase=-240, tstart=self.tstart,
        ...
        self.source += self.va.source + "\n" + self.vb.source + "\n" + self.vc.source

        if not self.hide_c:
            self.source += "\nC /FONTE TENSAO AC 3PH"