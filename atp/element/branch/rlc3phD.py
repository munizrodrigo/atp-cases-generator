...
class RLC_3ph_D(Element): #Verificar necessidade de bus_ref_pos e bus_ref_neg
    """
    Classe responsavel pela adicao de componentes RLC Trifasicos ligados em Delta, equivalentes_ramos lineares e linhas de transmissão simples.
    """
    def __init__(self, R1, L1, C1, bus_pos, phase_pos="A", R2=None, L2=None, C2=None, R3=None, L3=None, C3=None,
                 bus_ref_pos=None, bus_ref_neg=None, output_value=0, hide_c=False):
        """
        ...
        if not self.hide_c:
            if self.bus_ref_pos is None:
                self.branch = "C RLC TRIFASICO DELTA - POS:" + self.bus_pos.name + "\n"
            else:
                if self.bus_ref_neg is None:
                    self.branch = "C RLC TRIFASICO - POS:" + self.bus_pos.name + " - REF_POS: " + self.bus_ref_pos.name + "\n"
                else:
                    self.branch = "C RLC TRIFASICO - POS:" + self.bus_pos.name + " - REF_POS: " + self.bus_ref_pos.name + " - REF_NEG: " + self.bus_ref_neg.name + "\n"

        self.rlca = RLC(R=self.R1, L=self.L1, C=self.C1, bus_pos=self.bus_pos, phase_pos="A", bus_neg=self.bus_pos, phase_neg="B", output_value=self.output_value)
        self.rlcb = RLC(R=self.R2, L=self.L2, C=self.C2, bus_pos=self.bus_pos, phase_pos="B", bus_neg=self.bus_pos, phase_neg="C", output_value=self.output_value)
        self.rlcc = RLC(R=self.R3, L=self.L3, C=self.C3, bus_pos=self.bus_pos, phase_pos="C", bus_neg=self.bus_pos, phase_neg="A", output_value=self.output_value)

        self.branch = self.rlca.branch + "\n" + self.rlcb.branch + "\n" + self.rlcc.branch

        if not self.hide_c:
            self.branch += "\nC /RLC TRIFASICO DELTA"
