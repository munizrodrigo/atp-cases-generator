...
class CurrentAC(Element):
    """
    Classe responavel pela adicao de Fontes de Corrente AC.
    """
    def __init__(self, bus_pos, amp, freq=60, phase=0, phase_type=True, tstart=-1, tstop=100, phase_pos="A",
        ...
        if not self.hide_c:
            if self.bus_neg is None:
                self.source = "C FONTE CORRENTE AC - POS:" + self.bus_pos.name + "\n"
            else:
                self.source = "C FONTE CORRENTE AC - POS:" + self.bus_pos.name + " - NEG: " + self.bus_neg.name + "\n"
