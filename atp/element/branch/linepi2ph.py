...
class LinePi_2ph(Element): #Verificar ph1 e ph2
    """
    Classe responsavel pela adicao de linhas de transmissao bifasicas usando o modelo pi a parametros concentrados.
    """
    def __init__(self, R, L, C,  bus_pos, phase_pos="A", bus_neg=None, phase_neg="A", hide_c=False):
        ...
        if not self.hide_c:
            if self.bus_neg is None:
                self.branch = "C LINE PI BI PH - POS:" + self.bus_pos.name + "\n"
            else:
                self.branch = "C LINE PI BI PH - POS:" + self.bus_pos.name + " - NEG: " + self.bus_neg.name + "\n"
