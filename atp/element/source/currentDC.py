...
class CurrentDC(Element):
    """
    Classe responavel pela adicao de Fontes de Corrente DC.
    """
    def __init__(self, bus_pos, amp, tstart=1, tstop=100, phase_pos="A", bus_neg=None, phase_neg="A", hide_c=False):
        ...
        if not self.hide_c:
            if self.bus_neg is None:
                self.source = "C FONTE CORRENTE DC - POS:" + self.bus_pos.name + "\n"
            else:
                self.source = "C FONTE CORRENTE DC - POS:" + self.bus_pos.name + " - NEG: " + self.bus_neg.name + "\n"
